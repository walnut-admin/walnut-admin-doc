...
### 6. 自定义插件
- <WBaseLink preset="iconify">iconify</WBaseLink>图标相关的插件，具体看[这里](./icon.md)
- <WBaseLink preset="google-analytics">google-analytics</WBaseLink>插件，环境变量里配置了gaId才会加载，没有不加载，具体看[这里](../features/ga.md)
...
2. [@vitejs/plugin-legacy]（官方legacy插件）：
   ...
   - 目前默认是注释掉的状态，需要做老浏览器兼容自行打开注释，但需注意，可能会与其他插件冲突，导致构建错误。请一定要自行尝试。
...
5. [vite-plugin-bundle-obfuscator]（代码混淆插件）：
   ...
   - 注意：这个是最容易和懒加载或者CDN插件冲突的，真要做混淆需要花点时间研究一下。
...
8. [disable-devtool]（禁用开发工具插件）：
   - 功能：在生产环境下禁用浏览器的开发者工具，增加应用安全性。
   - 配置：通过环境变量 `VITE_BUILD_DISABLE_BROWSER_DEVTOOL` 控制是否启用（设为true即禁用）。

9. [sentry]（错误上报插件）：
   - 功能：将项目运行时的错误上报到Sentry平台，帮助开发者及时发现并定位问题。
   - 配置：需在Sentry平台创建项目并获取一系列配置项，例如dsn/org/project/authToken
   - 注意：authToken是涉密的配置，这种东西就不要随便上传到公开的仓库中了。
...
[unplugin-info]: https://github.com/yjl9903/unplugin-info
